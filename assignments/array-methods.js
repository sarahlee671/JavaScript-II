...
let allCaps = [];
const cap = function(first) {
    return first.toUpperCase();
}

allCaps = fullName.map(cap)
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result
let largeShirts = [];
const largeOrder = function(object) {
    return object.shirt_size === "L"
}
largeShirts = runners.filter(largeOrder)
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = [];
const calculation = function(accumu, donation) {
    return accumu + donation;
}
runners.forEach(function (object) {
    ticketPriceTotal.push(object.donation)
})
ticketPriceTotal = ticketPriceTotal.reduce(calculation)
console.log(ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.

// Problem 1 The Event Director wants to send some information before the event so he needs everyones email address on a list.
let emailList = []
runners.forEach(function (object) {
    let elist = object.email;
    emailList.push(elist)

})
console.log(emailList)


// Problem 2 The event director wants to give a gift for the people who donated over $200 dollars. Get a list of people with their information who donated more than $200.
let largeDonation = []
const largesum = function(object) {
    return object.donation > 200;
}
largeDonation = runners.filter(largesum)
console.log(largeDonation);


// Problem 3 The event director only wants the last name and company name on a list to put on the back of the shirts.
let newList = []
runners.forEach(function (object) {
    let compandlname = object.last_name + ": " + object.company_name;
    newList.push(compandlname)
})
console.log(newList);